...
const returnMovieTitle = (watchList) => {
    ...
    const filteredFilms = watchList.filter(movie => movie.Director === "Christopher Nolan" && parseFloat(movie.imdbRating) > 8)
    const movieNames = filteredFilms.map(movie => movie.Title);
    return movieNames
